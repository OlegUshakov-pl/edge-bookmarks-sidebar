...
// ===== Social network icons (Font Awesome) =====
const SOCIAL_ICONS = [
...
async function saveOrder() {
  ...
  // Move to "Other bookmarks" (id usually "2") with the new index.
  // To avoid breaking the structure, reorder items relative to each other.
  for (let i = 0; i < items.length; i++) {
    const id = items[i].dataset.id;
    try {
      await chrome.bookmarks.move(id, { parentId: '2', index: i });
    } catch (e) {
      // If moving to parentId "2" fails, try without a parentId
      try {
